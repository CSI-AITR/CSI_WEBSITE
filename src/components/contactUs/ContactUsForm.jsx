...
import { mailResponse } from '../../services/operations/contactUsApi';
...
function ContactUsForm() {
    ...
    const handleSendMessage=async (data)=>{
        setLoading(true);
        try{
            const response=await mailResponse(data);

            if(!response){
                throw new Error("ERROR WHILE SENDING CONTACT US MAIL")
            }
            else{
            ...
            console.log("ERROR WHILE SENDING CONTACT US MAIL FROM FRONTEND CONTACT PAGE: ",error);
        }
        setLoading(false);
    }
  ...
  return (
    <div>
        <form onSubmit={handleSubmit(handleSendMessage)} className='flex flex-col gap-7'>
            ...
            {/* query */}
            <div className="flex flex-col gap-2">
                <label htmlFor='message' className='label-style'>Query</label>
                <textarea
                    name='message'
                    id='message'
                    rows="7"
